...
export interface ModalManager {
  modals: (ModalProps | ReactNode)[];
  open: (props: ModalProps | ReactNode) => void;
  // openRaw: (modal: ReactNode) => void;
  pop: () => void;
}
...
export function Modal({
  ...
  return (
    <Wrapper onClose={onClose}>
      <div
        className={
          "relative bg-white rounded-xl shadow-xl w-full p-6 border border-gray-200 transform transition-all " +
          extraClassName
        }
      >
      ...
      {modals.map((modal, index) =>
        ...
        isValidElement(modal) ? (
          ...
          <Modal
            ...
            {...(modal as any)}
            onClose={() => {
              (modal as any).onClose && (modal as any).onClose();
              pop();
            }}
            onConfirm={() => {
              (modal as any).onConfirm && (modal as any).onConfirm();
              pop();
            }}
          />
        )
