...
import { DangerTextButton, PrimaryButton } from "../../components/buttons"
  ...
  return <div className="flex flex-col gap-2 items-center w-full">
    ...
    <div className="flex flex-col w-4/5 gap-4">
      ...
      <div className="flex flex-col gap-2">
        {apis.map((api) => (
          <div key={api.id} className="flex flex-row justify-between gap-2 border-b pb-0.5 border-slate-200">
            <div className="flex flex-col gap-1">
              <div className="text-lg font-bold">{api.name}</div>
              <div className="text-sm text-slate-500">{api.server_url}</div>
            </div>
            <div className="flex-col h-full self-end">
              {/* <div>1</div> */}
              <DangerTextButton className="text-sm" onClick={() => {
                modal.open({
